...
const PrometheusQueryCient = require('../prometheus/prometheus-query-client');
...
class PrometheusMonitor extends MonitorInterface {
    ...
    constructor(monitorConfig, interval) {
        ...
        this.prometheusQueryClient = new PrometheusQueryCient(monitorConfig.url);
    ...
    /**
     * Retrive the query client
     * @returns {PrometheusQueryCient} the query client
     */
    getQueryClient(){
    ...
    */
     ...
     * @returns {Map<string, string>[]} Array of Maps detailing the resource utilisation requests
     * @async
     */
    async getStatistics() {
        ...
        if (this.include) {
            ...
            for (const metricKey of Object.keys(this.include)) {
                ...
                // Retrieve base mapped statistics and cooerce into correct format
                const resultMap = PrometheusQueryHelper.extractStatisticFromRange(response, params.statistic);
