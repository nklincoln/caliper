...
class PrometheusQueryClient {
     ...
     */
    async configureTarget(type, query) {
        ...
        Logger.debug(`Prometheus query client configured to query ${this.requestParams.href}`);
    }
    ...
    async rangeQuery(queryString, startTime, endTime, step = 1) {
        ...
        this.configureTarget(RANGE, query);
    ...
    async query(queryString, timePoint) {
        ...
        this.configureTarget(SINGLE, query);
        return await this.retrieveResponse();
    }
    ...
    async getByEncodedUrl(getString) {
        ...
        this.configureTarget(NULL, getString);
        return await this.retrieveResponse();
    }
     ...
     * @param {JSON} qs the query options as a JSON object key:value pairing
