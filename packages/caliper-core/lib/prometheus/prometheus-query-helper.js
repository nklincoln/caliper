...
class PrometheusQueryHelper {
     ...
     * @returns {Map<string,value> | value} Either a map of values or single value depending on if the passed repsonse is a matrix or vector
     */
     ...
     */
    static extractStatisticFromRange(response, statType){
        ...
        case 'matrix':
        {
            ...
            const valuesMap = new Map();
            for (let result of response.data.result) {
                const name = (result.metric && result.metric.name) ? result.metric.name : 'unknown';
