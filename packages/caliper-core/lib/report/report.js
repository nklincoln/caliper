...
class Report {
     ...
     * @param {number} endTime end time for Prometheus data query, in millisectinds since epoch
    ...
    async getPrometheusResultValues(testLabel, round, startTime, endTime) {
        ...
        // Send Rate
        const txSubmittedQuery = `sum(caliper_txn_submitted{instance="${testLabel}", round="${round}"})`;
        const txSubmittedCountResponse = await this.queryClient.query(txSubmittedQuery, endQuery);
        const txSubmittedCount = txSubmittedCountResponse ?  PrometheusQueryHelper.extractFirstValueFromQueryResponse(txSubmittedCountResponse) : '-';
        const submitRate = (txSubmittedCount/(endQuery - startQuery)).toFixed(1);
        resultMap.set('Send Rate (TPS)', submitRate);
     ...
     */
    async buildRoundResourceStatisics(idx, label) {
